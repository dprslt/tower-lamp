...
//     worktree sees it. Paths may be whole files/dirs (.opencode/) or
//     individual ignored children of a partially-tracked dir (.opencode/
//     keeps its tracked skills/ but borrows the rest).
//   * Pre-seeds backend/node_modules and frontend/node_modules from the
//     primary worktree using a copy-on-write clone where the OS supports it
//     (macOS APFS clonefile via `cp -c`, Linux reflink via
//     `cp --reflink=auto`, Windows robocopy), so the `npm ci` that follows in
//     the Orca setup hook only reconciles this branch's delta.
//
...
//     node_modules is left for `npm ci` to build normally.
//
// Meant to run from Orca's worktree setup hook, *before* `npm ci`:
//
...
// primary worktree or outside a git checkout (e.g. CI).
...
// Per-package node_modules to pre-seed from the primary worktree.
const SEED_DIRS = ['backend/node_modules', 'frontend/node_modules'];
...
// Pre-seed a node_modules dir with a copy-on-write clone where the OS supports it.
const seedNodeModules = (rel) => {
  const src = path.join(primaryRoot, rel);
  const dest = path.join(worktreeRoot, rel);
  if (!fs.existsSync(src)) {
    log(`no source ${rel} — skipping pre-seed`);
    return;
  }
  if (fs.existsSync(dest)) {
    if (fs.existsSync(path.join(dest, '.package-lock.json')) || fs.existsSync(path.join(dest, '.yarn-integrity'))) {
      log(`${rel} already present — skipping pre-seed`);
      return;
    }
    log(`${rel} present but incomplete — purging for re-seed`);
  ...
  if (cow) {
    ...
    if (ok) {
      log(`pre-seeded ${rel} (${cow[0]})`);
      return;
    }
    fs.rmSync(dest, { recursive: true, force: true }); // clean a partial copy
  }
  log(`no copy-on-write path on ${process.platform} — leaving ${rel} for "npm ci"`);
};
...
try {
  ...
  shareConfig();
  for (const rel of SEED_DIRS) seedNodeModules(rel);
  log('done.');
} catch (err) {
  // Never block the `npm ci` that follows in the setup hook.
