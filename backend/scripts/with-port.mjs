...
//   - exports the resolved PORT to the child process (picked up by the backend
//     via process.env.PORT || 30008)
//   - substitutes the literal token `{PORT}` in any argument with the resolved port
//
// Examples:
//   node scripts/with-port.mjs tsx watch src/index.ts

import { execFileSync, spawn } from 'node:child_process';
import { createHash } from 'node:crypto';

const DEFAULT_PORT = '30008';
...
// Deterministic port in 30009..31007 (close to :30008, never collides with the default).
...
// `shell` is only needed on Windows (to launch `node.cmd`/`tsx.cmd`/`vite.cmd`);
